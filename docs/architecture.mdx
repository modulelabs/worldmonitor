...
All map layer toggle definitions — icon, localization key, fallback display label, and supported renderer types — are consolidated in a single shared registry (`src/config/map-layer-definitions.ts`). Each entry declares which map renderers support it via a `renderers: MapRenderer[]` field (e.g., `dayNight` is flat-only, `ciiChoropleth` is both flat and globe, AI Footprint point layers `startupHubs` / `techHQs` / `accelerators` / `cloudRegions` are svg + deck + globe alongside `datacenters`, and AI Footprint density/policy layers `convoHotspots` / `aiUsage` / `aiPolicy` are deck + globe — WebGL `pointsData` cylinders for usage/convo and country polygons for policy on the globe). The `full` and `tech` variant pickers both list Usage / Convo / Policy. Enabling a globe layer toggle rebuilds static catalogs before flush so Usage/Convo are not painted from empty arrays.
