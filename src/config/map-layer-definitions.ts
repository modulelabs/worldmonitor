...
export const LAYER_REGISTRY: Record<keyof MapLayers, LayerDefinition> = {
  ...
  // AI Footprint density / policy: DeckGL owns hexes + GeoJSON; GlobeMap paints
  // usage/convo markers and policy country polygons (see GlobeMap flush*).
  convoHotspots:            def('convoHotspots',            '&#128172;', 'convoHotspots',            'Convo Hotspots', ['deck', 'globe']),
  aiUsage:                  def('aiUsage',                  '&#128202;', 'aiUsage',                  'AI Usage', ['deck', 'globe']),
  aiPolicy:                 def('aiPolicy',                 '&#128220;', 'aiPolicy',                 'AI Policy', ['deck', 'globe']),
  ...
  // AI Footprint point layers: DeckGL + SVG already paint; GlobeMap mirrors
  // datacenters via htmlElementsData (see ensureStaticDataForLayer).
  startupHubs:              def('startupHubs',              '&#128640;', 'startupHubs',              'Startup Hubs', ['svg', 'deck', 'globe']),
  techHQs:                  def('techHQs',                  '&#127970;', 'techHQs',                  'Tech HQs', ['svg', 'deck', 'globe']),
  accelerators:             def('accelerators',             '&#9889;',   'accelerators',             'Accelerators', ['svg', 'deck', 'globe']),
  cloudRegions:             def('cloudRegions',             '&#9729;',   'cloudRegions',             'Cloud Regions', ['svg', 'deck', 'globe']),
...
export const LAYER_EXPLANATIONS: Partial<Record<keyof MapLayers, LayerExplanation>> = {
  ...
  convoHotspots: {
    ...
    purpose: 'Maps public attention around AI product-era timeline windows (Wikipedia pageviews by language edition; Google Trends / GDELT news / social when joined). Live merges AI-keyword tech digest items with article location or title→tech-hub inference.',
    source: 'Wikimedia Pageviews API (wiki channel). Google Trends CSVs and GDELT DOC ArtList are required joins for trends/news channels; social geotags still required. Live: NewsService list-feed-digest (tech variant) via src/services/convo-live-news.ts.',
    freshness: 'One 7-day window per product landmark (ChatGPT → Claude 3.5). Regenerated via scripts/join-wiki-ai-convo-hotspots.mjs (+ GDELT/Trends joins). Live digest refreshes when the layer turns on.',
    confidence: 'Wiki channel places language-edition pageviews on a primary-country centroid (language ≠ nation). Trends/news/social stay empty until cited joins exist — missing ≠ 0. Live pins only when lat/lon or hub match exists.',
    limitations: [
      'Google Trends interest-by-country CSVs not yet in scripts/_trends-ai-convo/ (API rate-limited from this host).',
      'GDELT news publisher-country join blocked/rate-limited — run join-gdelt-ai-convo-hotspots.mjs when API allows.',
      'No reliable public geotagged social corpus (Reddit velocity has no lat/lon).',
      'Live hub geo is title→hub inference (not article geotag) when digest items lack coordinates.',
    ],
    related: ['AI Footprint Product pillar', 'AI Usage (OWID)', 'Tech HQs', 'TimeNav product eras'],
    evidence: ['src/config/ai-convo-hotspots.ts', 'src/services/convo-live-news.ts', 'scripts/join-wiki-ai-convo-hotspots.mjs', 'scripts/join-gdelt-ai-convo-hotspots.mjs', 'REWRITE.md'],
  },
  aiUsage: {
    key: 'aiUsage',
    coverage: 'curated',
    category: 'AI / Usage',
    purpose: 'Hexagonal / cylindrical columns at country centroids for generative-AI usage / conversation density — OWID population shares (2025+) and WildChat conversation shares (boom-era). Flat map: ColumnLayer diskResolution:6 (HexagonLayer aggregation shaders fail under MapboxOverlay interleaved MapLibre). Globe: globe.gl pointsData cylinders.',
    source: 'OWID estimated share of people using generative AI; WildChat-1M country×quarter conversation share (ODC-BY); painted as extruded hex ColumnLayer over centroids.',
    freshness: 'WildChat quarters through 2024, then OWID waves (H1 2025, H2 2025, Q1 2026). Metrics are not interchangeable — tooltips name the source.',
    confidence: 'OWID = population usage share. WildChat = opt-in free-ChatGPT corpus geography (biased sample), not adult population %.',
    limitations: [
      'Not media/social attention — use Convo Hotspots for that.',
      'WildChat is not OWID-equivalent population usage; do not treat corpus % as adult adoption %.',
      'OpenAI Signals country tables remain a possible upgrade for boom-era population metrics.',
    ],
    related: ['AI Footprint Product pillar', 'Convo Hotspots', 'Tech HQs'],
    evidence: [
      'src/config/ai-conversation-growth.ts',
      'scripts/join-owid-ai-conversation-growth.mjs',
      'scripts/aggregate_wildchat_ai_usage.py',
      'REWRITE.md',
    ],
  },
  aiPolicy: {
    ...
    coverage: 'curated',
    category: 'AI / Policy',
    purpose: 'Country fills for implemented AI legislation and active policy discussion (bills / frameworks in development).',
    source: 'Cited COUNTRY_REGULATION_PROFILES in src/config/ai-regulations.ts (activeRegulations = implemented; proposedRegulations = discussion). EU profile expands to EU27 ISO2 for map paint.',
    freshness: 'Static catalog (lastUpdated per profile). Not live legislative scrapes.',
    confidence: 'Only countries with cited regulation ids paint. India/Australia stay blank in the current catalog (missing ≠ invent).',
    limitations: [
      'Not cyber IOCs — use Cyber Threats for Feodo/URLhaus geo.',
      'Voluntary ethics frameworks count only when listed under activeRegulations.',
      'National detail under EU Act is not separately profiled — members inherit the EU fill.',
    ],
    related: ['AI Footprint Policy pillar', 'RegulationPanel', 'AI Regulations catalog'],
    evidence: ['src/config/ai-regulations.ts', 'REWRITE.md'],
  },
};
...
const VARIANT_LAYER_ORDER: Record<MapVariant, Array<keyof MapLayers>> = {
  full: [
    ...
    'startupHubs', 'techHQs', 'convoHotspots', 'aiUsage', 'aiPolicy', 'accelerators', 'cloudRegions',
    'techEvents',
...
export const LAYER_SYNONYMS: Record<string, Array<keyof MapLayers>> = {
  ...
  hack: ['cyberThreats'],
  convo: ['convoHotspots'],
  conversation: ['convoHotspots'],
  owid: ['aiUsage'],
  'ai usage': ['aiUsage'],
  'ai policy': ['aiPolicy'],
  legislation: ['aiPolicy'],
  regulation: ['aiPolicy'],
  earthquake: ['natural'],
  volcano: ['natural'],
  tsunami: ['natural'],
